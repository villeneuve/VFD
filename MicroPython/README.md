...
### main.py
This script is still under development. It runs when the pico boots.  
It uses asyncio to run several tasks simultaneously :  
 ...
 and runs the web server showing vfd - motor - contactor status, actions possible: set the frequency, the date, the time, start/stop
  motor and contactor.  
 - Runs **read_sensors.py** and keeps sensors measured values in a list that is periodically printed. (To be completed only two DS18B20 temperature sensors for the time being)
...
-------------
  ...
  All these functions are still in development this is alpha release. 12/April/2026 
