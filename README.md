...
All these software are in the [MicroPython folder](./MicroPython).    

### 3.1.1 Main software

**main.py** runs at boot, call several other modules and launches 3 main asyncio tasks:  

 - One task is only listening to the REPL (USB) for incoming commands.  
 - One task is only taking care of the daily program: action when the time matches a programmed action.  
 - And the third task is doing everything:  
    - Driving the vfd with **vfdObj.py**. Additionally this module includes a bridge to allow direct passthrough ModBus commands from the Pi4 connected to the pico UART0.
 ...
 This bridge is a separate task running alone on core 1, all other tasks are running on core 0.  
    - Driving the lcd with **uselcd.py** (and a slightly modified version of lcd_Adafruit_16x2_RGB_i2c.py compared to the 
 one [here] [here](https://github.com/villeneuve/micropython-lcd-adafruit-16x2-rgb-i2c)).
  uselcd.py has a menu system to set the frequency, the date, the time, start/stop the motor and close/open the contactor)  
    - Runs **web_server.py**. When a push button is pressed starts the WiFi in access point mode, displays IP to connect at on the lcd, 
 and runs the web server showing vfd - motor - contactor status. Actions possible: set the frequency, the date, the time, start/stop
  motor and contactor, edit the daily program.  
    ...
    - Print every 20 seconds on the REPL all data from VFD and sensors in a Python list format. This list is read by Pi4 to update the supervision.  

### 3.1.2 Standalone software

Standalone software are MicroPython scripts that aren't used on the final project.  
...
On the Linux machine, the Raspberry Pi 4, we launch at boot (by the systemd unit **vfd-ctrl.service**) the script 
**ctrl_cde_sock.py**   
